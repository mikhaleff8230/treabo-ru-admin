...
export default function ProductSimpleForm({ initialValues }: IProps) {
  ...
  // Автозаполнение единицы измерения "шт." если поле пустое
  useEffect(() => {
    if (!unit && !initialValues?.unit) {
      setValue('unit', 'шт.');
    }
  }, [unit, initialValues, setValue]);
  ...
  return (
    <div className="my-5 flex flex-wrap sm:my-8">
      ...
      <Card className="w-full sm:w-8/12 md:w-2/3">
        ...
        {/* Показываем поле внешней ссылки только для типов OZON и Wildberries */}
        {selectedType && selectedType.name && (
          (selectedType.name.toLowerCase().includes('ozon') ||
           selectedType.name.toLowerCase().includes('wildberries')) ? (
            <Input
              label={
                selectedType.name.toLowerCase().includes('ozon')
                  ? t('form:input-label-preview-url-ozon')
                  : t('form:input-label-preview-url-wildberries')
              }
