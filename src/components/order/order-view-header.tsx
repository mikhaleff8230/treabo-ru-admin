...
export default function OrderViewHeader({
  ...
  const isPaymentActionPending = !isPaymentCOD && isOrderPending && normalizedPaymentStatus !== PaymentStatus.SUCCESS;
      ...
      <div className="mb-0 flex flex-col flex-wrap items-center justify-between gap-x-8 text-base font-bold text-heading sm:flex-row lg:flex-nowrap">
        ...
        >
          ...
          <div>
            ...
            <Badge
              text={t(normalizedPaymentStatus)}
              color={StatusColor(normalizedPaymentStatus)}
            />
          </div>
        </div>
      </div>
