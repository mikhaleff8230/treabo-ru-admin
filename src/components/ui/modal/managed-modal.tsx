...
const ProductGroupDeleteView = dynamic(
  () => import('@/components/product-group/product-group-delete-view')
);
const SkuDeleteView = dynamic(
  () => import('@/components/product-group/sku-delete-view')
);
...
const CreateProductGroupView = dynamic(
  () => import('@/components/product/create-product-group-view'),
  { 
    ssr: false,
    loading: () => (
      <div className="p-6">
...
function renderModal(view: MODAL_VIEWS | undefined, data: any) {
  switch (view) {
    ...
    case 'DELETE_PRODUCT_GROUP':
      return <ProductGroupDeleteView />;
    case 'DELETE_PRODUCT_SKU':
      return <SkuDeleteView />;
    case 'DELETE_TYPE':
