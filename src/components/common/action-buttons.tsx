  ...
  return (
    <div className="gap-8 inline-flex w-auto items-center">
      ...
      {onCopy && (
        <button
          onClick={onCopy}
          className="inline-flex items-center justify-center w-7 h-7 transition duration-200 hover:opacity-80 focus:outline-none"
          style={{ 
            backgroundColor: 'var(--toastify-spinner-color, #616161)',
            borderRadius: '8px'
          }}
          title={t('common:text-copy') || 'Копировать'}
        >
          <ClipboardIcon width={16} style={{ color: '#E0F316' }} />
        </button>
      )}
      {editModalView && (
        <button
      ...
      {detailsUrl && (
        <Link
          href={detailsUrl}
          className="ml-2 inline-flex items-center justify-center w-[30px] h-[30px] rounded-full transition duration-200 hover:opacity-80"
