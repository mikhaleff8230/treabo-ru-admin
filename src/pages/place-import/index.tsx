...
// import Checkbox from '@/components/ui/checkbox/checkbox'; // Используем обычные HTML чекбоксы
...
function PlaceImportPageContent() {
  ...
  // Групповое управление хештегами
  const [selectedPlaces, setSelectedPlaces] = useState<number[]>([]);
  const [bulkHashtags, setBulkHashtags] = useState<Array<{ id?: string; name: string } | string>>([]);
    ...
    try {
      ...
      for (let i = 0; i < maxCount; i++) {
        ...
        console.log('Place image data:', {
          title,
          matchedImage,
          availableImages: availableImages.slice(0, 3), // первые 3 для отладки
        });
  ...
  // Выбор/снятие выбора плейса
  const handlePlaceSelect = (index: number, checked: boolean) => {
    if (checked) {
      setSelectedPlaces(prev => [...prev, index]);
    } else {
      setSelectedPlaces(prev => prev.filter(i => i !== index));
    }
  };

  // Выбор/снятие выбора всех плейсов
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedPlaces(placesPreview.map((_, index) => index));
    } else {
      setSelectedPlaces([]);
    }
  };

  // Добавление хештегов к выбранным плейсам
  const handleBulkAddHashtags = () => {
    if (selectedPlaces.length === 0) {
      toast.error('Выберите плейсы для добавления хештегов');
      return;
    }

    if (bulkHashtags.length === 0) {
      toast.error('Выберите хештеги для добавления');
      return;
    }
    ...
    selectedPlaces.forEach(index => {
      if (updated[index]) {
        // Объединяем существующие хештеги с новыми, избегая дубликатов
        const existingNames = updated[index].hashtags.map(t => typeof t === 'string' ? t : t.name);
        const newHashtags = bulkHashtags.filter(t => {
          const tagName = typeof t === 'string' ? t : t.name;
          return !existingNames.includes(tagName);
        });

        updated[index].hashtags = [...updated[index].hashtags, ...newHashtags];
      }
    });

    setPlacesPreview(updated);
    toast.success(`Хештеги добавлены к ${selectedPlaces.length} плейсам`);

    // Очищаем выбор и bulk хештеги
    setSelectedPlaces([]);
    setBulkHashtags([]);
  };
        ...
        <Card>
          ...
          <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-3">
                <input
                  ...
                  checked={selectedPlaces.length === placesPreview.length && placesPreview.length > 0}
                  onChange={(e) => handleSelectAll(e.target.checked)}
                  className="rounded border-gray-300 text-accent focus:ring-accent"
                />
                <Label htmlFor="select-all-places" className="text-sm font-medium">
                  Выбрать все плейсы ({selectedPlaces.length} выбрано)
                </Label>
              </div>
              <Button
                size="small"
                onClick={handleBulkAddHashtags}
                disabled={selectedPlaces.length === 0 || bulkHashtags.length === 0}
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <Label className="text-xs text-blue-700">Хештеги для группового добавления</Label>
                <HashtagAutocomplete
                  value={bulkHashtags}
                  onChange={setBulkHashtags}
                  maxTags={20}
                  placeholder="Выберите хештеги для добавления к выбранным плейсам..."
                />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 max-h-[600px] overflow-y-auto">
            {placesPreview.map((place, index) => (
              <div
                key={index}
                className={`border rounded-lg p-4 ${selectedPlaces.includes(index) ? 'bg-blue-50 border-blue-300' : 'bg-gray-50'}`}
              >
                {/* Чекбокс для выбора плейса */}
                <div className="flex items-center space-x-2 mb-3 pb-2 border-b border-gray-200">
                  <input
                    type="checkbox"
                    id={`place-${index}`}
                    checked={selectedPlaces.includes(index)}
                    onChange={(e) => handlePlaceSelect(index, e.target.checked)}
                    className="rounded border-gray-300 text-accent focus:ring-accent"
                  />
                  <Label htmlFor={`place-${index}`} className="text-sm font-medium text-gray-700">
                    Плейс #{index + 1}
                  </Label>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="relative w-full h-32 bg-gray-200 rounded overflow-hidden">
                    <div className="flex items-center justify-center h-full text-gray-400 text-xs">
                      {place.imageName ? place.imageName.split('/').pop() : 'Нет фото'}
                    </div>
                  </div>
